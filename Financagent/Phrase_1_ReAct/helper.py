...
def update_summary(summary, messages, iteration, llm):
    ...
    system_prompt = SystemMessage(f'You are a very professional summarizer that could summarize old summary as well as new message and generate a new string of summary.')
    user_prompt = HumanMessage(f"""Please summarize this for me, here is the existing summary {summary} and this is the recent message {messages}.""")
...
#     output = f"""
#                 [CONTEXT]
#                 Year: {state.current_year}| Month: {state.current_month} | Category: {state.current_category} | Question type: {state.last_question_type}
#                 Known entities: {state.entities}
#                 Recent tools used: {state.tool_history[:-3]}
#                 [/CONTEXT]
#
#                 User: {user_input}
#             """
